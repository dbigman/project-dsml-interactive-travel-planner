...
def load_chromadb_collection(client, collection_name):
    ...
    the collection is successfully loaded, it returns the collection;
...
def retrieve_relevant_info(query, collection):
    ...
    retrieves the top three results that match the query. It logs the query
    being executed and the results obtained from the collection.
    ...
    Returns:
        list: A list of the top three results matching the query from the collection.
    """
...
def log_chat(user_input, model_reply):
    ...
    timestamp into a JSON Lines file. The chat data is appended to the file,
    allowing for continuous logging of chat interactions. If an error occurs
    during the file writing process, it is logged as an error.
...
def chat_with_llm(user_input):
    ...
    This function processes the user's input by retrieving relevant
    information from various collections, including municipalities,
    landmarks, and news. It combines the retrieved context into a single
    prompt for the language model. If no relevant context is found, it
    defaults to a general travel planning prompt about Puerto Rico. The
    function then sends the prompt to the OpenAI chat completions API and
    returns the model's reply.

    Args:
        user_input (str): The input provided by the user for which a response is needed.

    Returns:
        str: The response generated by the language model based on the user input
        and any relevant context retrieved.
    """
