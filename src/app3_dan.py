...
def chat_with_llm(user_input):
    """Chat with a language model using user input and relevant context.

    This function processes the user input to retrieve relevant information
    from various collections (municipalities, landmarks, and news). It
    combines the retrieved context into a single prompt for the language
    model. If no relevant context is found, it uses a general travel
    planning prompt. The function then interacts with the OpenAI API to
    generate a response based on the provided context and user input.
