...
def load_chromadb_collection(client, collection_name):
    ...
    case of an error during the retrieval process, it prints an error
...
def retrieve_relevant_info(query, collection):
    ...
    string and retrieves a set number of relevant documents. The number of
    results returned can be adjusted as needed. This is useful for
    applications that require fetching information based on user input or
    specific queries.

    Args:
        query (str): The query string used to search the collection.
        collection (CollectionType): The collection from which to retrieve documents.

    Returns:
        list: A list of relevant documents retrieved from the collection.
    """
...
def chat_with_llm(user_input):
    """Interact with a language model to answer user queries based on context.
    ...
    combines the retrieved information into a context string, which is then
    used to formulate a prompt for the language model. The model is
    instructed to answer the user's query using only the provided context.
    If the necessary information is not present, it will respond
    accordingly.

    Args:
        user_input (str): The user's query or input for which a response is needed.
