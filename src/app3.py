...
def load_chromadb_collection(client, collection_name):
    ...
    ChromaDB client. If successful, it returns the collection object. If an
    error occurs during the retrieval process, it prints an error message
    and returns None.
...
def retrieve_relevant_info(query, collection):
    """Retrieve relevant information from a specified collection.

    This function takes a query string and a collection object, then
    retrieves the top three results that match the query. It utilizes the
    collection's query method to perform the search and returns the results.

    Args:
        query (str): The query string used to search for relevant information.
        collection (object): The collection object that contains the data to be queried.

    Returns:
        list: A list of the top three results matching the query.
    """
...
def chat_with_llm(user_input):
    """Maintain conversation memory and log chat history.

    This function processes user input to retrieve relevant information from
    various collections, including municipalities, landmarks, and news. It
    combines this information into a context that is used to generate a
    response from an OpenAI language model. The function also maintains a
    history of the conversation by appending both the user input and the
    model's reply to the session state.
...
def log_chat(user_input, model_reply):
    ...
    timestamp into a JSON Lines file. Each entry is appended to the file,
    allowing for easy analysis of chat interactions over time.
